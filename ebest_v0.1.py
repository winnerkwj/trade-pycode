    ...
    def OnReceiveData(self, szTrCode):
        ...
        if szTrCode == "o3105":
            ...
            for i in range(occurs_count):
                기준일자 = self.GetFieldData("CIDBQ015000utBlock2", "BaseDt", i)
                예수금 = self.GetFieldData("CIDBQ015000utBlock2", "Dps", i)
            ...
            for i in range(occurs_count):
                ...
                해외선물청산손익금액 = self.GetFieldData("CIDBQ030000utBlock2", "AbrdFutsLqdtPnlAmt", i)
                해외선물수수료금액 = self.GetFieldData("CIDBQ030000utBlock2", "AbrdFutsCmsnAmt", i)
                평가자산금액 = self.GetFieldData("CIDBQ030000utBlock2", "EvalAssetAmt", i)
                해외선물위탁증거금액 = self.GetFieldData("CIDBQ030000utBlock2", "AbrdFutsCsgnMgn", i)
                해외선물인출가능금액 = self.GetFieldData("CIDBQ030000utBlock2", "AbrdFutsWthdwAbleAmt", i)
                해외선물주문가능금액 = self.GetFieldData("CIDBQ030000utBlock2", "AbrdFutsOrdAbleAmt", i)
                해외선물평가손익금액 = self.GetFieldData("CIDBQ030000utBlock2", "AbrdFutsEvalPnlAmt", i)
...
class XARealEvent:
    ...
    def OnReceiveRealData (self, trCode):
        ...
        if trCode == "OVH":
            ...
            매도호가건수4 = self.GetFieldData("OutBlock", "offernok")  # 여긴 혹시 오타인지 확인 필요
...
class XARealOrderEvent:
    ...
    def OnReceiveRealData(self, trCode):
        if trCode == "TC1":
            ...
            주문기간코드 = self.GetFieldData("OutBlock", "ordr_typ_prd_ccd")  # 0
            ...
            Object.주문접수_딕셔너너리[주문번호].update({"주문적용종료일자": 주문적용종료일자})
            ...
            위탁수수료 = self.GetFieldData("OutBlock", "ent_fee")
            사용자ID = self.GetFieldData("OutBlock", "userid")
            ...
            Object.주문체결_딕셔너너리[주문번호].update({"key": key})
...
class XingApi_Class(Object):
    ...
    def __init__(self):
        ...
        time.sleep(1.1) # 1초에 1회만 가능 (API 제약)
