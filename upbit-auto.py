...
class RateLimiter:
    def __init__(self, max_calls, period=1.0):
        ...
        :param period: Time period in seconds within which the max_calls are allowed.
        """
...
rsi_threshold = 10          # RSI가 11 이하일 때 매수
rsi_threshold_additional = 55  # 추가 매수를 위한 RSI 임계값 (55 이하)
initial_invest_ratio = 0.005  # 초기 투자 비율 (잔고의 0.5%)
target_profit_rate = 0.0030   # 목표 수익률 (0.3%)
stop_loss_rate = -0.60       # 손절매 기준 (-60%)
maintain_profit_rate = -0.0055 # 추가 매수 기준 수익률 (-0.5%)

# RSI 계산 주기 (초 단위)
rsi_calculation_interval = 5  # 5초마다 RSI 계산

# 추가 매수를 위한 최소 보유 시간 (초 단위)
min_hold_time_for_additional_buy = 80  # 90초

# 추가 매수 간 최소 시간 간격 (초 단위)
min_interval_between_additional_buys = 80  # 90초
