...
# 업비트 API 키 파일 경로 설정
# ------------------------------------------------------------
key_file_path = r'C:\Users\winne\OneDrive\바탕 화면\upbit_key.txt' #회사 개인
# ------------------------------------------------------------
# 로깅 설정
# ------------------------------------------------------------
...
# ------------------------------------------------------------
rsi_period = 14
rsi_threshold = 10 #rsi 트리거 기준
rsi_threshold_additional = 55
initial_invest_ratio = 0.05 #초기 매수 금액 (전체 금액의 N%로 진행)
target_profit_rate = 0.0030 #수익
stop_loss_rate = -0.6 #손절
maintain_profit_rate = -0.0055
rsi_calculation_interval = 60
min_hold_time_for_additional_buy = 60
                                ...
                                if elapsed_time >= min_interval_between_additional_buys:
                                    if additional_buy_count[ticker] < max_additional_buys:
                                        if rsi is not None and rsi < rsi_threshold_additional:
                                            ...
                                            if 55000 > krw_balance:
                                                invest_amount = krw_balance * (initial_invest_ratio * 2)
                                            else: invest_amount = krw_balance * initial_invest_ratio
