...
# 로깅 설정 (INFO 레벨로 체결 관련 메시지 명확히 출력)
...
balances_update_interval = 2.0  # balances 캐싱 주기 (초)
...
min_hold_time_for_additional_buy = 60*2
min_interval_between_additional_buys = 60*2
...
async def upbit_get_balances_async():
    ...
    except pyupbit.UpbitError as e:
...
async def watch_price():
    ...
    while True:
        ...
        try:
            ...
            logging.warning(f"웹소켓 연결 끊김, 재연결 시도: {e}", exc_info=True)
            await asyncio.sleep(1)
        except Exception as e:
